...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-blp_vlambda_mainline_torchlike_20260509}"
LOCAL_OUTPUT_ROOT_BASE="${LOCAL_OUTPUT_ROOT_BASE:-/mnt/e/autodl_results_new}"
OBSERVABLE_MODES="${OBSERVABLE_MODES:-abs complex_split}"
RESIDUAL_FORMS="${RESIDUAL_FORMS:-projected_vlambda}"
START_WITH_DATASET="${START_WITH_DATASET:-e10gh1}"
...
SEED="${SEED:-1234}"
TRAIN_SHUFFLE="${TRAIN_SHUFFLE:-1}"
SPECTRAL_SYNC_MODE="${SPECTRAL_SYNC_MODE:-pre_only}"
PYTHON_EXE="${PYTHON_EXE:-/home/kdshao/anaconda3/bin/python}"
DRY_RUN="${DRY_RUN:-0}"
...
BLP_ROOT="/mnt/e/DataPons_processed"

dataset_stems=(e10gh1 e10fV1 e10gw1 f12m01)

observable_file() {
  local dataset_stem="$1"
  local mode="$2"
  printf '%s/%s/pipeline1_reskoopnet_dictionary/%s_low50_high250_g2_%s_single.mat' "$BLP_ROOT" "$dataset_stem" "$dataset_stem" "$mode"
}
...
for idx in "${ordered_indices[@]}"; do
  ...
  for mode in "${mode_array[@]}"; do
    file="$(observable_file "$dataset_stem" "$mode")"
    if [[ ! -f "$file" ]]; then
      echo "Missing BLP observable file for ${dataset_stem} / ${mode}: ${file}" >&2
      exit 1
    fi
  done
done
...
for idx in "${ordered_indices[@]}"; do
  dataset_stem="${dataset_stems[$idx]}"
  dataset_experiment_name="${EXPERIMENT_NAME}_${dataset_stem}_seed${SEED}"
  ...
  for mode in "${mode_array[@]}"; do
    for residual_form in "${residual_array[@]}"; do
      ...
      data_file="$(observable_file "$dataset_stem" "$mode")"
      ...
      cmd=(
        ...
        "--data-root" "${BLP_ROOT}/${dataset_stem}"
      ...
      printf '=%.0s' {1..80}
      echo
      echo "Starting WSL BLP MLP ResKoopNet: $label"
      echo "Data file: $data_file"
      echo "Console log: $console_log"
      echo "Seed: $SEED"
      echo "Training schedule: shuffle=${TRAIN_SHUFFLE}, spectral_sync_mode=${SPECTRAL_SYNC_MODE}"
      printf '=%.0s' {1..80}
      echo
      ...
      if [[ "$MAX_PARALLEL" -gt 1 ]]; then
        ...
        (
          ...
          if [[ "$exit_code" -ne 0 ]]; then
            echo "WSL BLP MLP ResKoopNet failed for ${label} with exit code ${exit_code}" >&2
          fi
          exit "$exit_code"
        ) &
        active_pids+=("$!")
...
if [[ "$DRY_RUN" == "1" ]]; then
  ...
  echo
  echo "All requested WSL BLP MLP ResKoopNet runs finished successfully."
fi
