...
def parse_args():
    ...
    parser.add_argument('--run-name-base', default='cnn_obs_vlambda64')
    ...
    parser.add_argument('--residual-form', default='projected_vlambda', choices=['projected_kv', 'projected_vlambda'])
    parser.add_argument(
        '--training-policy',
        default='float64',
        choices=['float32', 'float64', 'mixed_float16'],
        help='Numeric policy passed into the CNN ResKoopNet solver.',
    )
...
def load_obs_and_border_idx(mat_path, limit_timepoints=None, dtype=np.float64):
    with h5py.File(mat_path, 'r') as f:
        ...
        obs = np.array(obs_dataset[:, :n_time]).T.astype(dtype, copy=False)
...
def restore_solver_checkpoint_for_export(solver, checkpoint_prefix, label):
    ...
    solver.eigenvectors = solver.eigenvectors_var.numpy().astype(complex_dtype, copy=False)
    solver.eigenvalues = solver.eigenvalues_var.numpy().astype(complex_dtype, copy=False)
...
def export_outputs(
    ...
    evalues = np.asarray(solver.eigenvalues.T, dtype=complex_dtype)
    kpm_modes = np.asarray(solver.compute_mode().T, dtype=complex_dtype)
    loss_array = np.asarray(losses, dtype=real_dtype)
    val_loss_array = np.asarray(val_losses, dtype=real_dtype)
    ...
    if outer_history:
        ...
        empty_history = np.array([], dtype=real_dtype)
    ...
    shared_outputs = {
        ...
        'final_eigvec_cond': np.asarray(
            _as_scalar_or_nan(getattr(solver, 'eigvec_cond', np.nan)),
            dtype=real_dtype,
        ),
    ...
    for i in range(num_chunks):
        ...
        efuns = np.asarray(solver.eigenfunctions(chunk_inputs), dtype=complex_dtype)
    ...
    for i in range(num_pair_chunks):
        ...
        psi_x = np.asarray(solver.dic.call(x_chunk).numpy().T, dtype=real_dtype)
        psi_y = np.asarray(solver.dic.call(y_chunk).numpy().T, dtype=real_dtype)
...
def main():
    ...
    load_dtype = np.float64 if (
        str(ARGS.training_policy).strip().lower() == 'float64'
        or str(ARGS.analysis_dtype).strip().lower() == 'float64'
    ) else np.float32

    obs, border_idx, n_time_total = load_obs_and_border_idx(
        mat_path,
        ARGS.limit_timepoints,
        dtype=load_dtype,
    )
