...
class KoopmanSolver(object):
    ...
    def __init__(
        ...
        self.spectral_complex_dtype = np.result_type(
            np.empty((), dtype=self.spectral_dtype),
            np.complex64
        )
    ...
    def _packed_residual_loss(self, y_true, y_pred):
        ...
        sq_sum = tf.reduce_sum(tf.square(diff), axis=-1)
        packed_dim = tf.cast(tf.shape(diff)[-1], self.training_real_dtype)
...
# inside build(), after you have self.data_x_train & self.data_y_train
    def _make_ds(self, x_array, y_array):
        ...
        def map_to_zero(x_batch, y_batch):
            ...
            zeros = tf.zeros((batch_size, self.output_dim), dtype=self.training_real_dtype)
        ...
        if checkpoint_path:
            if resume:
                ...
                if latest_checkpoint:
                    ...
                    self.K = K_var.numpy()
                    self.eigenvectors = self.eigenvectors_var.numpy().astype(self.spectral_complex_dtype, copy=False)
                    self.eigenvalues = self.eigenvalues_var.numpy().astype(self.spectral_complex_dtype, copy=False)
