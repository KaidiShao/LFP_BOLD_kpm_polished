...
DEFAULT_LOCAL_CHECKPOINT_PATH = '/mnt/e/autodl_results/e10gb1/checkpoints/batch2/model.ckpt'
...
class KoopmanSolver(object):
    ...
    def build_model(self):
        ...
        weighted_diff = tf.matmul(psi_diff, tf.cast(self.eigenvectors, tf.complex64))
        outputs = tf.concat(
...
# inside build(), after you have self.data_x_train & self.data_y_train
    def _make_ds(self, x_array, y_array):
        # 1) a simple Python generator that yields one (x,y) pair at a time
        x_np_dtype = self.training_input_dtype.as_numpy_dtype
        dict_dim = int(self.dic_func(
            tf.convert_to_tensor(x_array[:1], dtype=self.training_input_dtype)
        ).shape[-1])
        output_dim = dict_dim * 2

        def gen():
            for xi, yi in zip(x_array, y_array):
                yield xi.astype(x_np_dtype, copy=False), yi.astype(x_np_dtype, copy=False)

        # 2) declare the output types & shapes
        x_dtype, y_dtype = self.training_input_dtype, self.training_input_dtype
        x_shape = x_array.shape[1:]    # e.g. (604,) or (604,1) if 1D features
        y_shape = y_array.shape[1:]    # same logic for your y
        ...
        # 3) now map to ((x,y), zero) *per sample*, then batch
        def map_to_zero(xi, yi):
            zeros = tf.zeros((output_dim,), dtype=tf.float32)
            return (xi, yi), zeros
    ...
    def build(
        ...
        checkpoint_path = DEFAULT_LOCAL_CHECKPOINT_PATH
        checkpoint_dir = os.path.dirname(checkpoint_path)
        if not os.path.exists(checkpoint_dir):
            os.makedirs(checkpoint_dir)

        K_var = tf.Variable(self.K, name='K')
        eigenvectors_var = tf.Variable(self.eigenvectors, name='eigenvectors')
        ...
        if latest_checkpoint:
            ...
            self.K = K_var.numpy()
            self.eigenvectors = eigenvectors_var.numpy()
        ...
        for i in range(epochs):
            ...
            self.history = self.train_psi(
                self.model,
                epochs=Nepoch,
                callbacks=[EarlyStopping(monitor='val_loss', min_delta=.5, patience=25, verbose=1, mode='min')]
            )
        ...
        K_var.assign(self.K)
        eigenvectors_var.assign(self.eigenvectors)
