...
class KoopmanSolver(object):
    ...
    def __init__(self, dic, target_dim, reg=0.01):
        """Initializer
    ...
    def compute_K(self, dic, data_x, data_y, reg):
        ...
        """
        # determine dictionary dimension
        sample = data_x[:1]
        d = dic(sample).shape[-1]
...
# inside build(), after you have self.data_x_train & self.data_y_train
    def _make_ds(self, x_array, y_array):
        ...
        #    replace tf.float32 and shapes with your real dtypes/shapes
        x_dtype, y_dtype = tf.as_dtype(x_array.dtype), tf.as_dtype(y_array.dtype)
        ...
        def map_to_zero(xi, yi):
            psi_y = self.dic_func(tf.expand_dims(yi,0))    # shape [1, D]
            zeros = tf.zeros_like(psi_y)                    # shape [1, D]
            zeros = tf.squeeze(zeros, axis=0)               # shape [D]
            return (xi, yi), zeros
