...
result_dir = io_project.get_pipeline_stage_dir(output_root, cfg, 6, 'spkt_residual_cross_correlation');
lagged_data_dir = io_project.get_pipeline_stage_dir(output_root, cfg, 6, 'spkt_residual_lagged_cross_correlation');
figure_dir = io_project.get_pipeline_stage_dir(output_root, cfg, 6, 'figures_spkt_residual_lagged_cross_correlation');
result_file = '';

if exist(lagged_data_dir, 'dir') ~= 7
    mkdir(lagged_data_dir);
end
...
if isempty(result_file)
    L = dir(fullfile(result_dir, '*_spkt_all_koopman_residual_edmd_top*.mat'));
    if isempty(L)
...
params.save_dir = lagged_data_dir;
params.save_tag = sprintf('%s_lagged_pm%dbins', result_stem, params.max_lag_bins);
params.verbose = true;

out = compute_spkt_residual_lagged_cross_correlation(result_file, params);
...
ylabel('SPKT Channel');
title(sprintf('Peak |xcorr|: %s', strrep(feature_primary, '_', '\_')));
...
xlabel('Residual Mode Rank');
ylabel('SPKT Channel');
title(sprintf('Peak |xcorr|: %s', strrep(feature_secondary, '_', '\_')));
colormap(gca, parula(256));
colorbar;
caxis(mag_clim);
...
sgtitle(sprintf(['E10gb1 Lagged SPKT vs LFP Residual Cross-Correlation\n', ...
    'Positive lag means SPKT leads residual | max lag = %.2f s'], max(abs(out.lag_sec))), ...
    'FontWeight', 'bold');

png_file = fullfile(figure_dir, [params.save_tag, '_overview.png']);
fig_file = fullfile(figure_dir, [params.save_tag, '_overview.fig']);

exportgraphics(hfig, png_file, 'Resolution', 220);
savefig(hfig, fig_file);

fprintf('Saved lagged cross-correlation overview PNG:\n  %s\n', png_file);
fprintf('Saved lagged cross-correlation overview FIG:\n  %s\n', fig_file);
...
if top_n == 0
    text(0.5, 0.5, 'No finite lagged cross-correlations found', ...
