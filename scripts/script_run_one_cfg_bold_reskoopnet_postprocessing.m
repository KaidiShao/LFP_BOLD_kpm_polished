...
%   make_intrinsic_activation_maps = false;
%   make_intrinsic_roi_summary = false;
...
for i_run = 1:numel(runs)
    ...
    try
        ...
        if ctx.skip_existing
            ...
            [roi_result, attempted_roi_backfill] = backfill_bold_intrinsic_roi_bar_summaries(post_file, params);
            ...
            if attempted_roi_backfill
                roi_message = local_get_field(roi_result, 'message', '');
                if ~isempty(row_message) && ~isempty(roi_message)
                    row_message = sprintf('%s | %s', row_message, roi_message);
                elseif ~isempty(roi_message)
                    row_message = roi_message;
                end
                if strcmp(local_get_field(roi_result, 'status', ''), 'ok')
                    row_status = 'ok_intrinsic_roi_only';
                    fprintf('Backfilled intrinsic ROI summary:\n  %s\n', ...
                        local_get_field(roi_result, 'summary_dir', ''));
                else
                    fprintf('Intrinsic ROI summary already present or not changed.\n');
                end
            end
