...
TRANSFORMS = ["abs", "adaptive_envelope"]
...
def main() -> None:
    ...
    for transform in TRANSFORMS:
        ...
        if not args.summary_only:
            ...
            plot_perievent_examples(subset, bands, dt, transform)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--summary-only",
        action="store_true",
        help="Only save the three key summary figures: label counts and abs/adaptive strict candidate maps.",
    )
    parser.add_argument(
        "--include-dominant",
        action="store_true",
...
def configure_paths(args: argparse.Namespace) -> None:
    global DATASET, CONDITION, WORKSPACE, PROCESSED_ROOT, DATA_ROOT, P2_EVENT_MAT, P5_ROOT, OUT_ROOT, FIG_DIR, KS
...
def plot_perievent_examples(records: list[ComponentRecord], bands: dict[str, BandEvents], dt: float, transform: str) -> None:
    examples = choose_examples(records)
    if not examples:
        return
    fig, axes = plt.subplots(len(examples), 1, figsize=(16.0, 2.6 * len(examples)), sharex=True)
    if len(examples) == 1:
        axes = [axes]
    half_sec = 1.0
    ...
    for ax, rec in zip(axes, examples):
        activity = load_component_activity_for_record(rec)
        for band_name in BAND_ORDER:
            avg = mean_perievent(activity, bands[band_name].centers, half, max_events=1000)
        ...
        ax.set_title(
            f"{rec.method}_k{rec.k:02d} C{rec.component} | strict={rec.strict_label} | "
            f"T/G/R={rec.theta_effect_z:.2f}/{rec.gamma_effect_z:.2f}/{rec.ripple_effect_z:.2f} | "
            f"win={rec.envelope_window_sec:.3f}s",
            fontsize=9.5,
        )
        ax.grid(True, color="0.9")
    axes[-1].set_xlabel("time from P2 event peak (s)")
    axes[0].legend(frameon=False, ncol=3, loc="upper right")
    fig.suptitle(f"{DATASET}: selected component peri-event means | {transform}", fontsize=14, weight="bold")
    fig.tight_layout()
    fig.savefig(FIG_DIR / f"06_{transform}_selected_component_perievent_mean_by_band.png", dpi=180)
    plt.close(fig)


def choose_examples(records: list[ComponentRecord]) -> list[ComponentRecord]:
    selected: list[ComponentRecord] = []
    groups = [
        ("theta_selective", lambda r: r.strict_label == "theta_selective", lambda r: r.theta_effect_z),
        ("ripple_gamma_no_theta", lambda r: r.strict_label == "ripple_gamma_no_theta", lambda r: max(r.gamma_effect_z, r.ripple_effect_z)),
        ("gamma_selective", lambda r: r.strict_label == "gamma_selective", lambda r: r.gamma_effect_z),
        ("ripple_selective", lambda r: r.strict_label == "ripple_selective", lambda r: r.ripple_effect_z),
    ]
...
def load_component_activity_for_record(rec: ComponentRecord) -> np.ndarray:
    with h5py.File(rec.source_file, "r") as f:
        abs_activity = np.abs(np.asarray(f["result/core/temporal_components_time_by_comp"][rec.component - 1, :], dtype=np.float32))
        if rec.activity_transform == "abs":
            return abs_activity.astype(np.float64)
        # Single-component session-aware envelope; load session metadata from P2.
        _, _, _, session_start, session_end = load_p2_band_events()
        tmp = rms_envelope_sessionwise(
            abs_activity[None, :],
            session_start,
            session_end,
            np.asarray([rec.envelope_window_samples], dtype=int),
        )
        return tmp[0].astype(np.float64)
