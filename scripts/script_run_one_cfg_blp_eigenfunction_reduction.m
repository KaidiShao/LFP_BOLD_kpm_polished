...
%   lfp_activity_transform = 'abs_magnitude';
...
if exist('density_value_transform', 'var') && ~isempty(density_value_transform)
    params.density_value_transform = char(string(density_value_transform));
end
if exist('lfp_activity_transform', 'var') && ~isempty(lfp_activity_transform)
    params.lfp_activity_transform = char(string(lfp_activity_transform));
end
if exist('lfp_activity_window_policy', 'var') && ~isempty(lfp_activity_window_policy)
    params.lfp_activity_window_policy = char(string(lfp_activity_window_policy));
end
if exist('envelope_enable', 'var') && ~isempty(envelope_enable)
    params.envelope_enable = logical(envelope_enable);
end
if exist('envelope_policy', 'var') && ~isempty(envelope_policy)
    params.envelope_policy = char(string(envelope_policy));
end
if exist('envelope_alpha', 'var') && ~isempty(envelope_alpha)
    params.envelope_alpha = double(envelope_alpha);
end
if exist('envelope_min_window_sec', 'var') && ~isempty(envelope_min_window_sec)
    params.envelope_min_window_sec = double(envelope_min_window_sec);
end
if exist('envelope_max_window_sec', 'var') && ~isempty(envelope_max_window_sec)
    params.envelope_max_window_sec = double(envelope_max_window_sec);
end
if exist('envelope_fallback_window_sec', 'var') && ~isempty(envelope_fallback_window_sec)
    params.envelope_fallback_window_sec = double(envelope_fallback_window_sec);
end
if exist('component_timescale_weight_transform', 'var') && ~isempty(component_timescale_weight_transform)
    params.component_timescale_weight_transform = char(string(component_timescale_weight_transform));
end
if exist('component_timescale_top_modes', 'var') && ~isempty(component_timescale_top_modes)
    params.component_timescale_top_modes = double(component_timescale_top_modes);
end
...
fprintf('Density activity transform: %s (%s, value_transform=%s)\n', ...
    params.lfp_activity_transform, params.lfp_activity_window_policy, ...
    params.density_value_transform);
...
for i_run = 1:numel(runs)
    ...
    for i_method = 1:numel(method_specs)
        ...
        try
            ...
            if params.make_top30_window_plots
                try
                    ...
                    plot_paths.top30_window_error = "";
                catch ME_top30
                    plot_paths.top30_window_manifest_file = "";
                    plot_paths.top30_window_error = string(ME_top30.message);
                    fprintf(2, '  [WARN] %s top30 window plots failed:\n    %s\n', ...
                        method_tag, ME_top30.message);
                end
            else
                plot_paths.top30_window_manifest_file = "";
                plot_paths.top30_window_error = "";
        ...
        clear result plot_paths method_cfg spec row_i
    end
    clear EDMD_outputs concat_info source_info raw_cfg raw_prep raw_result run_info
end
