...
variability_params = struct();
variability_params.window_length_samples = 5000;
variability_params.keep_partial_window = false;
variability_params.top_k = 10;
variability_params.save_csv = true;
variability_params.force_recompute = false;

V = analyze_blp_consensus_event_variability_windows( ...
    cfg, ...
    output_root, ...
    C, ...
    variability_params, ...
...
fprintf('Saved per-window event-variability result to:\n  %s\n', V.save_file);
if ~isempty(V.csv_file)
    fprintf('Saved all-window CSV to:\n  %s\n', V.csv_file);
end
if ~isempty(V.top_csv_file)
    fprintf('Saved top-window CSV to:\n  %s\n', V.top_csv_file);
end

top_windows = V.top_windows_table;
if isempty(top_windows)
    error('Top-window table is empty in %s.', V.save_file);
end

save_cfg = struct();
save_cfg.save_dir = fullfile(output_root, cfg.file_stem, 'event_variability_windows', 'top_window_plots');
...
for i = 1:height(top_windows)
    ...
    file_stub = sprintf('rank_%02d_session_%02d_window_%03d', ...
        double(top_windows.variability_rank(i)), ...
    ...
    fig_title = sprintf(['Rank %d | session %d window %d | samples [%d,%d] | ' ...
        'transitions=%d | rate=%.3f Hz | richness=%d | Hnorm=%.3f'], ...
        double(top_windows.variability_rank(i)), ...
        double(top_windows.session_id(i)), ...
        double(top_windows.window_idx_in_session(i)), ...
        idx1, ...
        idx2, ...
        double(top_windows.transition_count(i)), ...
        double(top_windows.transition_rate_hz(i)), ...
...
disp('Top event-variability windows:');
disp(top_windows);
