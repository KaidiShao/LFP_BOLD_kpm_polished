...
%   dry_run = true;
%   backfill_phase = 'all';
%   p3_bold_modes = {'global_svd100','gsvd100_ds','HP','HP_svd100','roi_mean'};
...
autodl_bold_root = fullfile('E:', 'autodl_results_local', 'bold_wsl');
...
fprintf('  P5 methods/k: %s / k=%g\n\n', strjoin(p5_methods, ', '), p5_component_count);
...
for i_dataset = 1:numel(backfill_datasets)
    ...
    if any(strcmp(phase_set, 'post_bold'))
        action_rows = [action_rows; local_backfill_post_bold(item, processed_root, ...
            autodl_bold_root, minimal_bold_modes, dry_run, continue_on_error)]; %#ok<AGROW>
    end
end
...
function rows = local_backfill_post_bold(item, processed_root, autodl_bold_root, minimal_bold_modes, dry_run, continue_on_error)
rows = table();

available_modes = local_available_p4_bold_modes(autodl_bold_root, item.stem, minimal_bold_modes);
if isempty(available_modes)
    fprintf('No minimal P4 BOLD summaries available; skipping P7/P8. Run the WSL P4 launcher first.\n');
    return;
end
...
if ~isempty(missing_p7)
    ...
    run_vars.make_deconv_plot = true;
    run_vars.make_timescale_plot = true;
    run_vars.make_intrinsic_activation_maps = true;
    run_vars.make_intrinsic_roi_summary = true;
...
if ~isempty(missing_p8)
    ...
    run_vars.top_n = 5;
    run_vars.make_xcorr_figures = true;
    run_vars.make_activation_maps = true;
...
summary_dir = fullfile(processed_root, stem, 'pipeline5_summary_figures', ...
    'abs_projected_vlambda', 'eigenfunction_reduction', 'consensus_state_space');
for i = 1:numel(methods)
    pat = sprintf('%s_k%02d__efun_ssc__*.png', methods{i}, component_count);
...
density_dir = fullfile(processed_root, stem, 'pipeline5_dimred_thresholded_density', ...
    'abs_projected_vlambda', sprintf('umap_k%02d', component_count), 'mat', q_tag);
tf = ~isempty(dir(fullfile(density_dir, '*.mat')));
end
...
function modes = local_available_p4_bold_modes(autodl_bold_root, stem, requested_modes)
modes = {};
out_root = fullfile(autodl_bold_root, stem, 'mlp', 'outputs');
...
for i = 1:numel(requested_modes)
    ...
    for j = 1:numel(run_dirs)
        ...
        summary_files = dir(fullfile(run_dirs(j).folder, run_name, '*summary.mat'));
