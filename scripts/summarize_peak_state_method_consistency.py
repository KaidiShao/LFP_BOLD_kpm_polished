...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Only include method tags with these k values, e.g. --component-counts 8.",
    )
    parser.add_argument(
        "--include-shared",
        action="store_true",
...
def discover_runs(
    ...
    include_shared: bool,
    component_counts: Optional[Sequence[int]],
) -> List[RunSpec]:
    allowed_methods = {normalize_method_name(method) for method in methods}
    allowed_method_tags = {method.lower() for method in methods}
    allowed_k = None if component_counts is None else set(component_counts)
    if include_shared:
        allowed_methods.add("shared")
        allowed_method_tags.add("shared")
    ...
    for variant_dir in sorted(root.iterdir()):
        ...
        for method_dir in sorted(variant_dir.iterdir()):
            ...
            method = normalize_method_name(method_tag)
            component_count = parse_component_count(method_tag)
            if method not in allowed_methods and method_tag.lower() not in allowed_method_tags:
                continue
            if allowed_k is not None and component_count not in allowed_k:
                continue
            stats_files = sorted(method_dir.glob("*_peaks_stats.csv"))
            if not stats_files:
                continue
            stats_file = stats_files[0]
            run_id = f"{variant}/{method_tag}"
            runs.append(RunSpec(variant, method, method_tag, component_count, run_id, stats_file))
    return runs


def normalize_method_name(method_tag: str) -> str:
    clean = str(method_tag).strip().lower()
    for method in sorted(DEFAULT_METHODS, key=len, reverse=True):
        if clean == method or clean.startswith(f"{method}_") or clean.startswith(f"{method}-"):
            return method
    return clean


def parse_component_count(method_tag: str) -> Optional[int]:
    match = re.search(r"(?:^|[_-])k(\d+)(?:$|[_-])", str(method_tag).lower())
    if match is None:
        return None
        ...
        assignment_rows.append(
            {
                ...
                "component_count": "" if run.component_count is None else run.component_count,
                "run_id": run.run_id,
...
def write_summary_markdown(
    ...
    lines.append(f"- Component counts: `{metadata['component_counts']}`")
...
def main() -> int:
    ...
    metadata = {
        ...
        "component_counts": args.component_counts if args.component_counts is not None else "all",
