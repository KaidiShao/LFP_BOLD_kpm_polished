% Plot QC figures for BOLD SVD100 observable branches.
%
% Run from MATLAB:
%   cd('D:\Onedrive\ICPBR\Alberta\koopman_events\LFP_BOLD_kpm_polished')
%   run('scripts\script_plot_bold_svd100_qc.m')
...
for i_dataset = 1:numel(dataset_ids)
    ...
    for i_mode = 1:numel(observable_modes)
        ...
        fprintf('Plotting BOLD SVD100 QC: %s | %s\n', dataset_id, mode_name);
        [~, info] = plot_bold_pre_reskoopnet_qc(input_file, params);
        ...
        flat_files = copy_qc_files(info.output_files, project_root);
        fprintf('  Copied to project root: %s\n', strjoin(cellstr(flat_files), sprintf('\n                          ')));
    end
end

fprintf('\nBOLD SVD100 QC figures saved under:\n  %s\n', output_root);

function copied_files = copy_qc_files(source_files, target_dir)
