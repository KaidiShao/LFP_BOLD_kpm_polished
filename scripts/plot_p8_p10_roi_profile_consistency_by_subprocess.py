"""Plot P8/P10 ROI profile consistency stratified by P5 subprocess labels.

This is the ROI-consistency companion to the strict P2-band/P5 subprocess
analysis.  It expects ROI vectors exported with
``export_p8_p10_roi_profile_consistency_sources(...,
group_by_strict_subprocess=true)`` so that each ROI vector is already grouped
by the subprocess that produced the large xcorr hit.
"""
...
DEFAULT_FIGURE_DIR = (
    ...
    / "p8_p10_roi_profile_consistency_by_subprocess_20260603"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--p8-profile-csv",
        type=Path,
        default=DEFAULT_RESULTS_DIR / "p8_roi_profiles_by_subprocess_long.csv",
    )
    parser.add_argument(
        "--p10-profile-csv",
        type=Path,
        default=DEFAULT_RESULTS_DIR / "p10_roi_profiles_by_subprocess_long.csv",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Minimum n_datasets required for rows included in summary heatmaps/examples.",
    )
    parser.add_argument("--top-example-groups", type=int, default=32)
...
def aggregate_profiles(
    ...
    with csv_path.open("r", newline="", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            label_group = row.get("strict_label_group", "")
            ...
            if pipeline.upper() == "P8":
                ...
                info = {
                    ...
                    "strict_label_group": label_group,
...
def plot_summary_heatmaps(rows: list[dict[str, object]], figure_dir: Path, min_datasets: int) -> list[Path]:
    figure_dir.mkdir(parents=True, exist_ok=True)
    paths: list[Path] = []
    rows = [r for r in rows if int(r.get("n_datasets", 0)) >= min_datasets]
    ...
    for pipeline in ("P8", "P10"):
        ...
        matrix = average_matrix(
            ...
            lambda r: str(r.get("strict_label_group", "")),
            obs_feature_rows,
            list(LABEL_ORDER),
        )
        if matrix:
            path = figure_dir / f"{pipeline.lower()}_roi_consistency_by_observable_feature_subprocess.png"
            plot_heatmap(
                matrix,
                obs_feature_rows,
                list(LABEL_ORDER),
                f"{pipeline} ROI consistency | observable x BOLD efun type x subprocess | n_datasets >= {min_datasets}",
                path,
            )
            paths.append(path)

        for label_group in LABEL_ORDER:
            label_subset = [r for r in subset if r.get("strict_label_group") == label_group]
            matrix = average_matrix(
                label_subset,
                lambda r: f"{RUN_TAG_LABELS.get(str(r.get('run_tag', '')), str(r.get('observable', '')))} | {r.get('feature_family', '')}",
            ...
            if matrix:
                path = figure_dir / f"{pipeline.lower()}_roi_consistency_by_lfp_method_k__{label_group}.png"
                plot_heatmap(
                    matrix,
                    obs_feature_rows,
                    method_cols,
                    f"{pipeline} ROI consistency | LFP method-k | {label_group} | n_datasets >= {min_datasets}",
                    path,
                )
                paths.append(path)
        ...
        if pipeline == "P10":
            for label_group in LABEL_ORDER:
                ...
                if matrix:
                    path = figure_dir / f"p10_roi_consistency_by_bold_p9_method_k__{label_group}.png"
                    plot_heatmap(
                        matrix,
                        obs_feature_rows,
                        method_cols,
                        f"P10 ROI consistency | BOLD P9 method-k | {label_group} | n_datasets >= {min_datasets}",
                        path,
                    )
                    paths.append(path)
    return paths
...
def plot_example_profiles(
    ...
    ranked = sorted(
        [r for r in rows if int(r.get("n_datasets", 0)) >= min_datasets],
...
def main() -> None:
    ...
    label_groups = set(args.label_groups)
    ...
    write_csv(args.results_dir / f"p8_roi_profile_consistency_by_subprocess{suffix}.csv", p8_rows)
    write_csv(args.results_dir / f"p10_roi_profile_consistency_by_subprocess{suffix}.csv", p10_rows)
    write_csv(
        args.results_dir / f"p8_p10_roi_profile_consistency_by_subprocess_ranked{suffix}.csv",
    ...
    heatmap_paths = plot_summary_heatmaps(all_rows, figure_dir, args.plot_min_datasets)
    example_paths = plot_example_profiles(
        ...
        args.plot_min_datasets,
    )
    print(f"P8 subprocess aggregated groups: {len(p8_vectors)}")
    print(f"P8 consistency rows: {len(p8_rows)}")
    print(f"P10 subprocess aggregated groups: {len(p10_vectors)}")
    print(f"P10 consistency rows: {len(p10_rows)}")
    print(f"Method-k scope: {args.method_k_scope}")
    print(f"Results dir: {args.results_dir}")
    print(f"Figure dir: {figure_dir}")
    print(f"Plot min datasets: {args.plot_min_datasets}")
