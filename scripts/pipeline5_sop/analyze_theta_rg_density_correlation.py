...
def main() -> None:
    ...
    if args.legacy_figures:
        print(f"Legacy figures {FIG_ROOT}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--label-suffix", default=LABEL_SUFFIX)
    parser.add_argument("--density-condition", default=DENSITY_CONDITION)
    parser.add_argument(
        "--result-root",
        default="",
        help="Output directory for density-correlation CSVs. Default is under workspace/results.",
    )
    parser.add_argument(
        "--figure-root",
        default="",
        help="Only used with --legacy-figures.",
    )
    parser.add_argument(
        "--legacy-figures",
        action="store_true",
        help="Also write the older exploratory figure set under figure-root.",
    )
    return parser.parse_args()


def configure_paths(args: argparse.Namespace) -> None:
    global WORKSPACE, PROCESSED_ROOT, LABEL_SUFFIX, DENSITY_CONDITION, RESULT_ROOT, FIG_ROOT

    WORKSPACE = Path(args.workspace)
    PROCESSED_ROOT = Path(args.processed_root)
    LABEL_SUFFIX = str(args.label_suffix)
    DENSITY_CONDITION = str(args.density_condition)
    RESULT_ROOT = (
