% Canonical e10gb1 consensus-state-diversity plotting entry point.
% This script always uses the 6000-sample global-window state-diversity result.
...
window_result_file = fullfile(output_root, cfg.file_stem, 'consensus_state_diversity_windows', ...
    [cfg.file_stem, '_consensus_state_diversity_windows_6000samp_globalwin.mat']);

save_cfg = struct();
save_cfg.save_dir = fullfile(output_root, cfg.file_stem, 'consensus_state_diversity_windows', 'top_window_plots');
...
W = S.W;
local_validate_e10gb1_state_diversity_result(W, window_result_file);
...
function local_validate_e10gb1_state_diversity_result(W, window_result_file)
if ~isfield(W, 'window_mode') || ~strcmpi(char(string(W.window_mode)), 'global')
    error(['%s is not a global-window state-diversity result. ', ...
