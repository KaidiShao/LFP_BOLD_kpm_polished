% Canonical interactive state-diversity plotting entry.
%
% Usage from MATLAB:
%   cfg_name = 'E10gH1';
...
%
% Role:
%   - canonical single-dataset plotting entry for consensus-state-diversity windows
%   - loads a saved state-diversity result, then delegates figure export to
%     the canonical state-diversity plotting function
%
...
pipeline_params = build_blp_consensus_state_pipeline_params();
if exist('window_length_samples', 'var') && ~isempty(window_length_samples)
    pipeline_params.window_params.window_length_samples = double(window_length_samples);
end
if exist('window_mode', 'var') && ~isempty(window_mode)
    pipeline_params.window_params.window_mode = char(string(window_mode));
end
...
if exist(window_result_file, 'file') ~= 2
    error('Consensus-state-diversity result file does not exist:\n  %s', window_result_file);
end
...
P = export_top_consensus_state_diversity_window_plots(cfg, output_root, W, params);
