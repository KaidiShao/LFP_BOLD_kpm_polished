function [hfig, plot_info] = plot_blp_segment_with_spectrogram_and_koopman( ...
    cfg, output_root, time_range_sec, cmap_in, freq_lim, clim, event_input, band_colors, koopman_input)
...
% heatmaps for a chosen time window:
...
%   .base_plot_cache    optional cached raw/event/spectrogram data for reuse
...
show_events = (nargin >= 7) && ~isempty(event_input);
if show_events && nargin < 8
    band_colors = [];
end

if nargin < 9 || isempty(koopman_input)
...
if isfield(cfg, 'plot')
    if isfield(cfg.plot, 'trace_scale'), trace_scale = cfg.plot.trace_scale; end
    if isfield(cfg.plot, 'trace_clip'), trace_clip = cfg.plot.trace_clip; end
    if isfield(cfg.plot, 'within_gap'), within_gap = cfg.plot.within_gap; end
    if isfield(cfg.plot, 'between_gap'), between_gap = cfg.plot.between_gap; end
    if isfield(cfg.plot, 'trace_linewidth'), trace_linewidth = cfg.plot.trace_linewidth; end
end

% -------------------- raw data --------------------
if isfield(koopman_input, 'base_plot_cache') && ~isempty(koopman_input.base_plot_cache)
    base_plot_cache = koopman_input.base_plot_cache;
    used_cached_base = true;
else
    base_plot_cache = local_prepare_base_plot_cache( ...
