...
% Resolve observable settings and choose the input data branch.
...
% Dispatch one concrete observable-construction path.
switch lower(params.mode)
    ...
    case 'slow_band_power'
        [obs, observable_info, model] = build_bold_slow_band_power_observables(X, D, params);

    case 'svd'
        [obs, observable_info, model] = build_svd_observables(X, D, params);

    otherwise
        error('Unsupported params.mode: %s', params.mode);
end

% Pack a standardized observable struct for downstream code.
O = struct();
