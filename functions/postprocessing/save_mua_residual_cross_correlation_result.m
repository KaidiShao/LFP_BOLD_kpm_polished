function result = save_mua_residual_cross_correlation_result(result, save_dir)
%SAVE_MUA_RESIDUAL_CROSS_CORRELATION_RESULT Save MUA xcorr MAT/CSV outputs.
...
channel_label = local_filename_safe(result.source.blp_channel_selection_label);
band_label = local_filename_safe(result.source.blp_band_selection_label);
tag = local_build_save_tag(result, save_dir, source_label, channel_label, band_label);
...
function tag = local_build_save_tag(result, save_dir, source_label, channel_label, band_label)
if contains(local_filename_safe(save_dir), source_label, 'IgnoreCase', true)
    tag = sprintf('%s_mua_%s_%s_top%d', ...
        result.cfg.file_stem, channel_label, band_label, height(result.mode_table));
else
    tag = sprintf('%s_%s_mua_%s_%s_top%d', ...
        result.cfg.file_stem, source_label, channel_label, band_label, height(result.mode_table));
end
tag = local_filename_safe(tag);
if strlength(fullfile(save_dir, [tag '.mat'])) > 240
    tag = local_filename_safe(sprintf('%s_mua_%s_%s_top%d', ...
        result.cfg.file_stem, channel_label, band_label, height(result.mode_table)));
end
end
