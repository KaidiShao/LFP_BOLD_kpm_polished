...
dataset_dirs = local_roi_dataset_dir_candidates(run_info);
roi_dir = '';
tried_dirs = cell(size(dataset_dirs));
for i_dir = 1:numel(dataset_dirs)
    tried_dirs{i_dir} = fullfile(params.datapons_root, dataset_dirs{i_dir}, 'roits');
    if exist(tried_dirs{i_dir}, 'dir') == 7
        roi_dir = tried_dirs{i_dir};
        break;
    end
end
if isempty(roi_dir)
    error('ROI directory not found. Tried: %s', strjoin(tried_dirs, '; '));
end
...
function dataset_dirs = local_roi_dataset_dir_candidates(run_info)
raw_names = {};
if isfield(run_info, 'dataset_id')
    raw_names{end + 1} = char(string(run_info.dataset_id)); %#ok<AGROW>
end
if isfield(run_info, 'dataset_stem')
    raw_names{end + 1} = char(string(run_info.dataset_stem)); %#ok<AGROW>
end

dataset_dirs = {};
for i_name = 1:numel(raw_names)
    name = raw_names{i_name};
    if isempty(name)
        continue;
    end
    dataset_dirs{end + 1} = name; %#ok<AGROW>
    dataset_dirs{end + 1} = lower(name); %#ok<AGROW>
    token = regexp(lower(name), '^k13m(\d+)$', 'tokens', 'once');
    if ~isempty(token)
        dataset_dirs{end + 1} = sprintf('K13.m%s', token{1}); %#ok<AGROW>
    end
end
dataset_dirs = unique(dataset_dirs, 'stable');
end
