function out = compute_spkt_residual_lagged_cross_correlation(result_input, params)
%COMPUTE_SPKT_RESIDUAL_LAGGED_CROSS_CORRELATION Lagged SPKT-vs-residual cross-correlation.
%
% Positive lag means SPKT leads residual:
%   xcorr_lag(k) = corr(spkt(t), residual(t + k))
%
% Inputs
...
params = local_apply_defaults(params, base_result_file, base_result);

if params.verbose
    fprintf('Lagged cross-correlation source:\n  %s\n', base_result_file);
end
...
function params = local_apply_defaults(params, base_result_file, base_result)
...
if ~isfield(params, 'save_dir') || isempty(params.save_dir)
    if isstruct(base_result) && isfield(base_result, 'cfg') && ...
            isfield(base_result.cfg, 'file_stem') && ~isempty(base_result.cfg.file_stem)
        output_root = io_project.get_project_processed_root();
        if isfield(base_result, 'params') && isstruct(base_result.params) && ...
                isfield(base_result.params, 'output_root') && ...
                ~isempty(base_result.params.output_root)
            output_root = base_result.params.output_root;
        end
        params.save_dir = io_project.get_pipeline_stage_dir( ...
            output_root, base_result.cfg, 6, 'spkt_residual_lagged_cross_correlation');
    elseif ~isempty(base_result_file)
...
if ~isfield(params, 'save_tag') || isempty(params.save_tag)
    if ~isempty(base_result_file)
        ...
        params.save_tag = 'spkt_residual_lagged_xcorr';
    end
end
end
