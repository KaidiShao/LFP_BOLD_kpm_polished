...
%
% Current mainline dimred density expands over method x k by default:
%   svd/nmf/mds/umap x k03:k08
...
for i_kind = 1:numel(requested_kinds)
    ...
    try
        source = repmat(local_empty_density_source(), 0, 1);
        switch kind
            case 'event_density'
                source = local_resolve_event_density_source( ...
                    params.processed_root, dataset_stem, dataset_id);
            case 'raw_abs_density'
                condition_tag = local_condition_with_suffix( ...
                    'abs_projected_vlambda', params.blp_density_condition_suffix);
                source = local_resolve_explicit_pipeline5_density_source( ...
                    params.processed_root, dataset_stem, dataset_id, ...
                    'raw_thresholded_density', condition_tag, '', ...
                    params.blp_density_threshold_ratio, ...
                    local_explicit_source_name('raw_abs', '', params.blp_density_threshold_ratio, ...
                    params.blp_density_condition_suffix), ...
                    'thresholded_density');
            case 'dimred_abs_density'
                condition_tag = local_condition_with_suffix( ...
                    'abs_projected_vlambda', params.blp_density_condition_suffix);
                source = local_resolve_explicit_pipeline5_dimred_density_sources( ...
                    params.processed_root, dataset_stem, dataset_id, ...
                    condition_tag, params.blp_dimred_method_tags, ...
                    params.blp_density_threshold_ratio, 'dim_abs', ...
                    params.blp_density_condition_suffix);
            case 'raw_complex_split_density'
                condition_tag = local_condition_with_suffix( ...
                    'complex_split_projected_vlambda', params.blp_density_condition_suffix);
                source = local_resolve_explicit_pipeline5_density_source( ...
                    params.processed_root, dataset_stem, dataset_id, ...
                    'raw_thresholded_density', condition_tag, '', ...
                    params.blp_density_threshold_ratio, ...
            ...
            case 'dimred_complex_split_density'
                ...
                source = local_resolve_explicit_pipeline5_dimred_density_sources( ...
                    params.processed_root, dataset_stem, dataset_id, ...
                    condition_tag, params.blp_dimred_method_tags, ...
                    params.blp_density_threshold_ratio, 'dim_csplit', ...
                    params.blp_density_condition_suffix);
        ...
        density_sources = [density_sources; source(:)]; %#ok<AGROW>
    catch ME
...
params = local_set_default(params, 'blp_density_condition_suffix', '');
params = local_set_default(params, 'blp_dimred_method_tags', local_default_dimred_method_tags());
if ~isfield(params, 'blp_dimred_method_tags') || isempty(params.blp_dimred_method_tags)
    params.blp_dimred_method_tags = local_default_dimred_method_tags();
end
params.blp_dimred_method_tags = local_normalize_method_tags(params.blp_dimred_method_tags);
if ~isfield(params, 'blp_dimred_method_tag') || isempty(params.blp_dimred_method_tag)
    params.blp_dimred_method_tag = params.blp_dimred_method_tags{end};
end
end


function tags = local_default_dimred_method_tags()
...
function tags = local_normalize_method_tags(tags)
if isempty(tags)
    tags = {};
elseif ischar(tags)
    tags = {char(tags)};
elseif isstring(tags)
    tags = cellstr(string(tags(:)).');
elseif iscell(tags)
    tags = cellstr(string(tags(:)).');
else
    error('blp_dimred_method_tags must be a string-like list.');
end
end


function condition_tag = local_condition_with_suffix(condition_tag, suffix)
suffix = char(string(suffix));
if isempty(suffix)
    return;
end
condition_tag = sprintf('%s_%s', condition_tag, suffix);
condition_tag = regexprep(condition_tag, '[^a-zA-Z0-9_\-]+', '_');
condition_tag = regexprep(condition_tag, '_+', '_');
condition_tag = regexprep(condition_tag, '^_+|_+$', '');
end
...
function sources = local_resolve_explicit_pipeline5_dimred_density_sources( ...
        processed_root, dataset_stem, dataset_id, condition_tag, ...
        method_tags, threshold_ratio, source_prefix, source_suffix)
if nargin < 8
    source_suffix = '';
end
method_tags = cellstr(string(method_tags(:)).');
sources = repmat(local_empty_density_source(), 0, 1);
for i_method = 1:numel(method_tags)
    method_tag = method_tags{i_method};
    source = local_resolve_explicit_pipeline5_density_source( ...
        processed_root, dataset_stem, dataset_id, ...
        'dimred_thresholded_density', condition_tag, method_tag, ...
        threshold_ratio, ...
        local_explicit_source_name(source_prefix, method_tag, threshold_ratio, source_suffix), ...
        'dimred_thresholded_density');
...
function name = local_explicit_source_name(kind, method_tag, threshold_ratio, suffix)
if nargin < 4
    suffix = '';
end
...
suffix = char(string(suffix));
if ~isempty(suffix)
    suffix = regexprep(lower(suffix), '[^a-z0-9]+', '_');
